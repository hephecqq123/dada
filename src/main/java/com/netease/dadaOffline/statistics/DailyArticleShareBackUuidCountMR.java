...
public class DailyArticleShareBackUuidCountMR extends MRJob {
    ...
    public boolean init(String date) {
        ...
        outputList.add(DirConstant.STATISTICS_TODC_DIR + "articleShareBackUuidCount/" + date);
        return true;
    }
    ...
    public int run(String[] args) throws Exception {
        ...
        // reducer
        job1.setReducerClass(ArticleShareBackUuidCountReducer.class);
    ...
    public static class ZyLogMapper extends Mapper<LongWritable, Text, Text, IntWritable> {
        ...
        public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
        	...
        	String uuid = map.get(ZyLogParams.uid);
    ...
    public static class ArticleShareBackUuidCountReducer
            extends Reducer<Text, IntWritable, Text, IntWritable> {
    	private Text outKey = new Text();
        private IntWritable outputValue = new IntWritable();
        private  int share_uuid_all = 0;
        private int back_uuid_all =0;
        @Override
        protected void reduce(Text key, Iterable<IntWritable> values, Context context)
                throws IOException, InterruptedException {
       
            if (key.toString().startsWith("share_")){
            	share_uuid_all++;
            }else if (key.toString().startsWith("back_")){
            	back_uuid_all++;
            }
		...
		protected void cleanup(
			...
			outKey.set("share_uuid_"+DaDaConstants.column_all);
            outputValue.set(share_uuid_all);
            context.write(outKey, outputValue);
    		outKey.set("back_uuid_"+DaDaConstants.column_all);
            outputValue.set(back_uuid_all);
            context.write(outKey, outputValue);
		}
