...
function errorAlarm(){
	message=$1
	completeMessage=[Error]_dada_${yesterday}_${message}
	echo completeMessage
	sh ${ALARM_DIR}/alarm.sh $completeMessage ${ALARM_DIR}/contactlist.txt
}

function waringAlarm(){
	message=$1
	completeMessage=[Waring]_dada_${yesterday}_${message}
	echo completeMessage
	sh ${ALARM_DIR}/alarm.sh $completeMessage ${ALARM_DIR}/contactlist.txt
}
...
function hadoopDirPrepare(){
	...
	dt=$2
	now=`date "+%H%M%S"`
	hadoopDateDirExist=`${HADOOP} fs -ls $hadoopDir | grep -v _|grep -c $dt`
